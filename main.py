...
def screen_speakers(stdscr):
    ...
    # Reproducir solo en canal RIGHT - usar misma estrategia que LEFT
    success = play_audio_test('right', kill_pipewire=False)
    
    if not success:
        log_debug("RIGHT falló con pipewire activo, reintentando sin pipewire...")
        success = play_audio_test('right', kill_pipewire=True)
        ...
        try:
            ...
            if test['name'] == 'WiFi':
                ...
                try:
                    ...
                    stdscr.addstr(12, 10, "[S] = Sí, ejecutar WIPE", curses.color_pair(3))
                    stdscr.addstr(13, 10, "[Cualquier otra] = Saltar WIPE")
                    stdscr.refresh()
                except: pass
                
                key = stdscr.getch()
                if key in [115, 83]:  # Solo 's' o 'S'
